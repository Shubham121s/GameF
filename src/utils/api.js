...
api.interceptors.response.use(
  ...
  (error) => {
    ...
    // Handle 404 errors more gracefully for unimplemented endpoints
    if (error.response.status === 404) {
      console.log("⚠️ API Endpoint not found:", error.config?.url);
      // Add helpful message for unimplemented endpoints
      if (error.response.data?.message?.includes("not found")) {
        console.log(
          "💡 This endpoint may not be implemented yet in the backend"
        );
      }
    }
